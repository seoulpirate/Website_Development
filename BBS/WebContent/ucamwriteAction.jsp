...
<body>
	<%
		...
		if (session.getAttribute("ccode") != null) {
			courseID = (String) session.getAttribute("ccode");
		}
		// 글쓰기는 로그인이 되어 있어야 가능. >> 수정바람. 이미 로그인된 상태이다.	
		...
		try {
			...
			if (uBbsTitle == null || uBbsContent == null) {
				...
				int result = ubbsDAO.write(uBbsTitle, userID, courseID, uBbsContent, fileName);
				if (result == -1) {
					PrintWriter script = response.getWriter();
					script.println("<script>");
